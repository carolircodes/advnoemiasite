...
class ClientMergeService {
  ...
  async mergeClients(input: MergeClientsInput): Promise<MergeClientsOutput> {
    ...
    try {
      ...
      const conflictCheck = await this.checkChannelConflicts(
        input.sourceClientId,
        input.targetClientId
      );
      if (conflictCheck.hasConflicts) {
        result.errors?.push(
          `Channel conflicts detected: ${conflictCheck.conflicts.join(", ")}`
        );
        return result;
      }

      const channelsResult = await this.moveClientChannels(
        input.sourceClientId,
        input.targetClientId
      );
      result.movedChannels = channelsResult.moved;

      const sessionsResult = await this.moveConversationSessions(
        input.sourceClientId,
        input.targetClientId
      );
      result.movedSessions = sessionsResult.moved;

      const pipelineResult = await this.moveClientPipeline(
        input.sourceClientId,
        input.targetClientId
      );
  ...
  async linkChannelToExistingClient(
    input: LinkChannelToExistingClientInput
  ): Promise<LinkChannelToExistingClientOutput> {
  ...
  private async moveClientChannels(
    sourceClientId: string,
    targetClientId: string
  ...
  private async moveConversationSessions(
    sourceClientId: string,
    targetClientId: string
  ): Promise<{ moved: number }> {
    try {
  ...
  private async moveClientPipeline(
    sourceClientId: string,
    targetClientId: string
