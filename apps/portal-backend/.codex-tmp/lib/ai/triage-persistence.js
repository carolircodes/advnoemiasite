...
exports.saveTriageData = saveTriageData;
/**
 * Salvar dados da triagem no banco de dados
 */
const triage_persistence_1 = require("../services/triage-persistence");
async function saveTriageData(input, conversationState, classification) {
    ...
    const userId = input.metadata?.userId ||
        ...
        input.conversationState?.userId ||
        'unknown';
    await triage_persistence_1.triagePersistence.saveTriageData(input.metadata?.sessionId || 'unknown', triageData, {
...
function buildStructuredReport(input, state, classification) {
    ...
    return {
        resumo_caso: data.problema_principal ||
        ...
        sinais_urgencia: data.nivel_urgencia && data.nivel_urgencia !== 'baixa'
            ? [`Urgência ${data.nivel_urgencia}`]
            : [],
        documentos_mencionados: data.tipos_documentos || [],
        documentos_pendentes: data.tem_documentos === false ? ['Documentos ainda não enviados'] : [],
        ...
        pipeline_id: typeof input.metadata?.pipelineId === 'string' ? input.metadata.pipelineId : null,
