...
export async function GET(request: NextRequest) {
  try {
    ...
    if (leadsError) {
      logEvent("ANALYTICS_LEADS_QUERY_ERROR", { 
    ...
    if (eventsError) {
      logEvent("ANALYTICS_EVENTS_QUERY_ERROR", { 
        error: eventsError.message 
      }, "error");
      
      return NextResponse.json(
        { error: "Erro ao buscar dados de eventos" },
        { status: 500 }
      );
    }
    ...
    const sources: SourcePerformance[] = Array.from(sourceMap.values()).map(source => ({
      ...source,
    ...
    const topics: TopicPerformance[] = Array.from(topicMap.values()).map(topic => ({
      ...topic,
    ...
    const content: ContentPerformance[] = Array.from(contentMap.values()).map(content => ({
      ...content,
      conversionRate: content.leadsGenerated > 0 ? (content.conversions / content.leadsGenerated) * 100 : 0
    })).sort((a, b) => b.conversions - a.conversions);
    ...
    logEvent("ANALYTICS_SERVER_ERROR", {
